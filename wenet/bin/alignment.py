...
def create_ctc_graph1(target, blank_idx):
    """Build gtn graph.
...
if __name__ == '__main__':
    ...
    with torch.no_grad(), open(args.result_file, 'w',
        ...
        for batch_idx, batch in enumerate(ali_data_loader):
            ...
            T, C = ctc_probs.shape
            g_emissions = gtn.linear_graph(T, C, ctc_probs.requires_grad)
            cpu_data = ctc_probs[:T].cpu().contiguous()
            g_emissions.set_weights(cpu_data.data_ptr())
            g_criterion = create_ctc_graph(target, 0)
            g_path = gtn.viterbi_path(gtn.intersect(g_emissions, g_criterion))
            alignment = g_path.labels_to_list()
            print(alignment)
