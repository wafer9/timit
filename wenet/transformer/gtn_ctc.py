...
class GTNCTCLossFunction(torch.autograd.Function):
    ...
    def forward(ctx, log_probs, targets, ilens, blank_idx=0, reduction="none"):
        ...
        paths_pad = pad_sequence([torch.LongTensor(p) for p in paths], True, -1)
        return torch.mean(loss.cuda() if log_probs.is_cuda else loss), paths_pad


    @staticmethod
    def backward(ctx, grad_output, paths):
